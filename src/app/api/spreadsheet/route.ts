...
export async function GET(request: Request) {
    ...
    const response = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${sheetName}!A:Z?${isRaw ? 'valueRenderOption=UNFORMATTED_VALUE&' : ''}key=${process.env.GOOGLE_API_KEY}`,
        {
            headers: {
                Referer: 'https://kb-is8244gxh-tlemcencrma20-2660s-projects.vercel.app',
    ...
    // ✅ Ajout du message personnalisé de débogage
    const responseWithDebug = {
        ...data,
        _debug: {
            message: `✅ API exécutée avec succès pour la feuille "${sheetName}"`,
